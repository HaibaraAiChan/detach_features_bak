...
def run(args, device, data):
	# Unpack data
	g, nfeat, labels, n_classes, train_nid, val_nid, test_nid = data
	in_feat = len(nfeat[0])
	nvidia_smi_list=[]
	...
	model = SAGE(in_feat, args.num_hidden, n_classes, args.num_layers, F.relu, args.dropout, args.aggre)
	...
	input_nodes_list=[108680, 54299, 27111, 13568, 6791, 3398, 1696 ]
	for inp in range(len(input_nodes_list)):
		print('Epoch ' + str(inp))
		from dgl.data.utils import load_graphs
		full_batch_subgraph =list(load_graphs('./DATA/ideal_mini_subgraph/'+args.dataset+'_'+str(inp)+'_subgraph.bin',[0]))
	...
	for epoch, full_batch_subgraph in enumerate(full_batch_sub_graph_data_list): # args.epochs
		...
		for step, (input_nodes, seeds, blocks) in enumerate(block_dataloader):
			...
			batch_inputs, batch_labels = load_block_subtensor(nfeat, labels, blocks, device)
		...
		print('current Epoch training on CPU Time(s): {:.4f}'.format(train_end_toc - train_start_tic ))
		...
		print('\t\t current epoch data from CPU to GPU time:%.8f s' % (transer_time/1000))
		...
		print('\t\t current epoch total CPU time without block dataloader generation:%.8f s' % (epoch_CPU_time ))
	...
	# print('\ttotal avg epoch total cpu time:%.8f s' % (total_avg_step_time ))
	
	
	total_avg_epoch_data_trans_time = sum(t_step_data_trans_time_list[out_indent:]) / len(t_step_data_trans_time_list[out_indent:])
	print('\t avg epoch data from cpu -> GPU time\t:%.8f s' % (total_avg_epoch_data_trans_time/1000))
	total_avg_epoch_gpu_time = sum(t_step_GPU_train_time_list[out_indent:]) / len(t_step_GPU_train_time_list[out_indent:])
	...
	print()
	print('='*100)
	train_acc = evaluate(model, g, nfeat, labels, train_nid, device)
	print('train Acc: {:.6f}'.format(train_acc))
	test_acc = evaluate(model, g, nfeat, labels, test_nid, device)
