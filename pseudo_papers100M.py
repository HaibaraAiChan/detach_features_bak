...
def run_papers100M(args, device, data):
	# Unpack data
	n_classes, nfeat, in_feats, g, labels, train_nid, val_nid, test_nid = data
	# n_classes, train_g, val_g, test_g, train_nfeat, train_labels, val_nfeat, val_labels, test_nfeat, test_labels = data
	...
	print(get_memory('----------------------------------------after sampler'))
	full_batch_size = len(train_nid)
	full_batch_dataloader = dgl.dataloading.NodeDataLoader(
		g,
		train_nid,
		...
		drop_last=False,
		num_workers=args.num_workers)
	print(get_memory('----------------------------------------after full_batch_dataloader'))
	model = SAGE(in_feats, args.num_hidden, n_classes, args.num_layers, F.relu, args.dropout, args.aggre)
	model = model.to(device)
	loss_fcn = nn.CrossEntropyLoss()
	optimizer = optim.Adam(model.parameters(), lr=args.lr)
	...
	for epoch in range(args.num_epochs):
		...
		for full_batch_step, (input_nodes, output_seeds, full_batch_blocks) in enumerate(full_batch_dataloader):
			print(get_memory('----------------------------------------in loop of  full_batch_dataloader'))
			
			
			ssss_time = time.time()
			block_dataloader, weights_list,time_collection = generate_dataloader(g, full_batch_blocks,  args)
			total_generate_time = time.time() - ssss_time
			print(get_memory('----------------------------------------after generate_dataloader'))
			#-------------after data loading----------------------------------------------------------------------------------------------------------
			...
			for step, (input_node, seeds, blocks) in enumerate(block_dataloader):
				print("\n   ***************************     step   " + str(step) + " mini batch block  *************************************")
				print(get_memory('----------------------------------------loop in block_dataloader'))
				src_collection.append(len(input_node))
				torch.cuda.synchronize()
				start.record()
				...
				batch_inputs, batch_labels = load_subtensor(nfeat, labels, seeds, input_nodes, device)
				# batch_inputs, batch_labels = load_blocks_subtensor(g, labels, blocks, device)
				blocks = [block.int().to(device) for block in blocks]
...
def main(args):
	...
	if args.dataset=='karate':
		...
		if args.flag_papers100M_load_bin == 0:
			preprocess_papers100M(args)
			print('write success!')
			return 
		else:
			print('start preapre---------------------')
			...
			device = "cuda:0"
			print('start run --------------------------------')
			temp=run_papers100M(args, device, data)
			return
...
if __name__=='__main__':
	...
	argparser.add_argument('--flag_papers100M_load_bin', type=int, default=1)
