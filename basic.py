...
def run(args, device, data):
	...
	t_step_time_list = []
	epoch_CPU_without_data_loading=[]
	avg = 0
	iter_tput = []
	for epoch in range(args.num_epochs):
		...
		for step, (input_nodes, seeds, blocks) in enumerate(train_dataloader):
			...
			CPU_without_data_loading.append(time.time()-step_s)
		...
		print('\t\t current epoch total CPU time \t:%.8f s' % (epoch_CPU_time ))
		t_step_time_list.append(epoch_CPU_time)
		
		epoch_CPU_without_data= sum(CPU_without_data_loading)
		print('\t\t current epoch total CPU time without datloader \t:%.8f s' % (epoch_CPU_without_data ))
		epoch_CPU_without_data_loading.append(epoch_CPU_without_data)
	...
	avg_epoch_CPU_time = sum(t_step_time_list[out_indent:]) / len(t_step_time_list[out_indent:])
	print('\t avg_epoch_CPU_time total cpu time:%.8f s' % (avg_epoch_CPU_time ))
	
	avg_epoch_total_cpu_train_time = sum(epoch_CPU_without_data_loading[out_indent:]) / len(epoch_CPU_without_data_loading[out_indent:])
	print('\n\t avg epoch training total cpu time without dataloader\t:%.8f s' % (avg_epoch_total_cpu_train_time ))
